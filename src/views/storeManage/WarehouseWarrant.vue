  ...
  methods: {
    ...
    async confirm() {
      ...
      try {
        ...
        if (cvencode0 == "" && this.bodyData.length > 0)
          cvencode0 = (this.bodyData[0] as any).cvencode;
        if (cvencode0 == "" && this.multipleSelection.length > 0)
          cvencode0 = (this.multipleSelection[0] as any).cvencode;

        if (cvencode0 != "" && this.multipleSelection.length > 0) {
          var k = this.multipleSelection.findIndex(
            (data: any) => data.cvencode != cvencode0
          );
          ...
          if (k != -1) {
            ...
            this.multipleSelection.forEach(async (item: any) => {
              ...
              if (index == -1) {
                let res = await this.SqlWork(
                  "select",
                  "insert into wlzh_asntemp(iposid,cinvcode,cuser_id,cvencode,dtime,bqty,dArriveDate) select id,cinvcode,'" +
                    this.SysInfo.cUserId +
                    "','" +
                    item.cvencode +
                    "',getdate()," +
    ...
    async testSqlWork2() {
      try {
        ...
        let res = await this.SqlWork(
          "select",
          "select 'aaaa' cvencode,'cccc' cvouchtype,'dddd' vouchDate"
        );
        console.log(res);
    ...
    async getTableData() {
      try {
        ...
        let res = await this.SqlWork(
          "select",
          "select top 15 convert(varchar(50),rd.vouchDate,23) date,rd.vouchCode name,rds.cinvcode,rds.iquantity cwhname  from rdrecord01 rd join rdrecords01 rds on rd.id=rds.id where rd.vouchDate>='2024-01-01'"
        );
