      ...
      <el-row class="el-row">
        ...
        >
          ...
          <el-table-column
            ...
            :sortable="true"
          />
          <el-table-column prop="qty" label="归还数量" width="150">
          ...
          <el-table-column prop="crdcode" label="收发类别编码" width="120" />
          <el-table-column
            prop="cDepName"
            label="部门"
            width="120"
  ...
  async mounted() {
    ...
    if (this.$route.query.vouchCode) {
      ...
      const res2 = await this.SqlWork(
        "select",
        "select cDepName,id,autoid,sort,csource,cvouchtype,vouchcode ccode,vouchdate ddate,crdname,cmemo,cvencode,cvenname,cwhcode,cwhname,cposcode,cinvcode,cinvname,cinvstd,ccomunitcode,ccomunitname,cbatch,inqty,outqty qty,cbmemo from wlzh_v_DaoJuDetail_gh " +
          filterStr
      );
      console.log(
        res1,
  ...
  methods: {
    ...
    async loadData() {
      try {
        ...
        let sql = "select  * from wlzh_v_daojugh_consult";
...
VALUES('02','U8','','${this.headerData.ddate}','${item.id}','${item.autoid}','${item.cposcode}','${this.headerData.cwhname}','${item.cvencode}','${item.cinvcode}','${item.qty}','${this.SysInfo.cUserId}',getdate(),'${item.cmemo}','${item.cdepcode}','${item.cbatch}','${GID}')`
          );
        });
        // 等待所有循环操作完成
